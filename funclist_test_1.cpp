...
int main() {
    ...
    assert(flist::as_string(flist::map(flist::as_string,
                                       flist::map(flist::rev, std::ref(ll)))) ==
           "[[2.5;1];[char[];1;2.5];[string;s;c];[]]");

    auto flat_ll = flist::flatten(std::ref(ll));
    cout << flist::as_string(std::ref(flat_ll)) << endl;
    assert(flist::as_string(flat_ll) == "[1;2.5;2.5;1;char[];c;s;string]");

    auto l7 = flist::filter(
        [](auto x) {
    ...
    auto dupa = flist::cons(1, flist::cons(2.5, flist::empty));
    auto dupa_fliter = flist::filter(
                [](auto x) {
                    return x == 1;
                },
                dupa);
    cout << "printing dupa "
         << flist::as_string(dupa_fliter)
         << endl;

    auto custom_l = [i = 2](auto f, auto a) {
        return f("This is ", f(std::ref(i), f(' ', f("custom call", a))));
    };
    cout << flist::as_string(custom_l) << endl;
    assert(flist::as_string(custom_l) == "[This is ;2; ;custom call]");
    assert(flist::as_string(flist::rev(custom_l)) ==
           "[custom call; ;2;This is ]");
    assert(flist::as_string(flist::rev(flist::rev(custom_l))) ==
           "[This is ;2; ;custom call]");
    ...
    auto seqN = flist::map([&](auto l) {return l(add, 0);},
    flist::rev(std::ref(list_of_lists))); assert(seqN([](int i, int a)
    {assert(i == a); return a + 1;}, 0) == N);
}
